...
public static class Translation
{
    ...
    {
        byte GetBinaryCode(char c)
        {
            return Convert.ToByte(Convert.ToByte(c) - 48);
        } // why 48? check ASCII table

        var bytes = Enumerable.Repeat((byte)0xFF, 8).ToArray();
        userCode = userCode.Length % 2 == 0 ? userCode : "0" + userCode;
        for (var i = 0; i < userCode.Length; i += 2) bytes[i / 2] = (byte)((GetBinaryCode(userCode[i]) << 4) | GetBinaryCode(userCode[i + 1]));
    ...
    public static byte[] ToByteArray(bool[] logicState)
    {
        ...
        for (var i = 0; i < binaryStateLength; i++)
    ...
    public static bool[] ToBooleanArray(byte[] binaryState)
    {
        ...
        for (var i = 0; i < binaryState.Length; i++)
        for (var j = 0; j < ByteSize; j++)
        {
            var decimalValue = (byte)(0b0000_0001 << j);
            if ((binaryState[i] & decimalValue) == decimalValue) logicState[i * ByteSize + j] = true;
        }
