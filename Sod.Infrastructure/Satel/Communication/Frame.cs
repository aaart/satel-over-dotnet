...
{
    public static byte[] Create(Command cmd, byte[] userCode, byte[] data)
    {
        return Create((byte)cmd, userCode, data);
    }

    public static byte[] Create(Command cmd, params byte[] data)
    {
        return Create((byte)cmd, Array.Empty<byte>(), data);
    }

    public static (byte crcHigh, byte crcLow) Crc(byte cmd, byte[] data)
    {
        byte CrcHigh(ushort crc)
        {
            return (byte)(crc >> 8);
        }

        byte CrcLow(ushort crc)
        {
            return (byte)crc;
        }

        ushort crc = 0x147A;
        foreach (var element in new[] { cmd }.Concat(data))
        {
