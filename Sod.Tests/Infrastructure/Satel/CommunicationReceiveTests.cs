...
public class CommunicationReceiveTests
{
    ...
    public async Task GivenBinaryData_WhenFrameIsCorrect_ThenValidResult(byte[] frame)
    {
    ...
    public async Task GivenBinaryData_WhenFrameIsIncorrect_ThenInvalidCrcResult(Command expectedCommand, byte[] frame)
    {
        var segment = new ArraySegment<byte>(frame);
        var socketReceiver = new MockSocketReceiver(() => Task.FromResult((frame.Length, segment)));
    ...
    public async Task GivenBinaryData_WhenFrameIsIncorrect_ThenInvalidFrameResult(Command expectedCommand, byte[] frame)
    {
        var segment = new ArraySegment<byte>(frame);
        var socketReceiver = new MockSocketReceiver(() => Task.FromResult((frame.Length, segment)));
        var (receiveStatus, data) = await ReceiveAsync(socketReceiver, expectedCommand);
