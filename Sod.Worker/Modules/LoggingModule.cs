...
public class LoggingModule : Module
{
    protected override void Load(ContainerBuilder builder)
    {
        ...
        builder.ComponentRegistryBuilder.Registered += (_, args) => { args.ComponentRegistration.PipelineBuilding += (_, pipeline) => { pipeline.Use(new LoggingMiddleware()); }; };
    }
    ...
    public class LoggingMiddleware : IResolveMiddleware
    {
        public void Execute(ResolveRequestContext ctx, Action<ResolveRequestContext> next)
        {
            ...
            if (instance is ILoggingCapability loggingCapability) loggingCapability.Logger = ctx.Resolve<SerilogLoggerFactory>().CreateLogger(instance.GetType());
        }
